...
public class ConnPoolByRoute extends AbstractConnPool {
    ...
    protected BasicPoolEntry getEntryBlocking(HttpRoute route, Object state, long timeout, TimeUnit tunit, WaitingThreadAborter aborter) throws ConnectionPoolTimeoutException, InterruptedException {
        ...
        while (entry == null) {
            ...
            if (this.log.isDebugEnabled()) {
                this.log.debug(RequestParameters.LEFT_BRACKETS + route + "] total kept alive: " + this.freeConnections.size() + ", total issued: " + this.leasedConnections.size() + ", total allocated: " + this.numConnections + " out of " + this.maxTotalConnections);
            }
            ...
            try {
                ...
                if (this.log.isDebugEnabled()) {
                    this.log.debug("Available capacity: " + rospl.getCapacity() + " out of " + rospl.getMaxEntries() + " [" + route + "][" + state + RequestParameters.RIGHT_BRACKETS);
                }
                if (hasCapacity && this.numConnections < this.maxTotalConnections) {
                    ...
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Need to wait for connection [" + route + "][" + state + RequestParameters.RIGHT_BRACKETS);
                    }
    ...
    public void freeEntry(BasicPoolEntry entry, boolean reusable, long validDuration, TimeUnit timeUnit) {
        ...
        if (this.log.isDebugEnabled()) {
            this.log.debug("Releasing connection [" + route + "][" + entry.getState() + RequestParameters.RIGHT_BRACKETS);
        }
        this.poolLock.lock();
        try {
            if (this.shutdown) {
    ...
    protected BasicPoolEntry getFreeEntry(RouteSpecificPool rospl, Object state) {
        ...
        while (!done) {
            ...
            if (entry != null) {
                if (this.log.isDebugEnabled()) {
                    this.log.debug("Getting free connection [" + rospl.getRoute() + "][" + state + RequestParameters.RIGHT_BRACKETS);
                }
                this.freeConnections.remove(entry);
                if (entry.isExpired(System.currentTimeMillis())) {
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Closing expired free connection [" + rospl.getRoute() + "][" + state + RequestParameters.RIGHT_BRACKETS);
                    }
                    closeConnection(entry);
                    rospl.dropEntry();
                    this.numConnections--;
                ...
                if (this.log.isDebugEnabled()) {
                    this.log.debug("No free connections [" + rospl.getRoute() + "][" + state + RequestParameters.RIGHT_BRACKETS);
                }
            }
        }
        this.poolLock.unlock();
    ...
    protected BasicPoolEntry createEntry(RouteSpecificPool rospl, ClientConnectionOperator op) {
        if (this.log.isDebugEnabled()) {
            this.log.debug("Creating new connection [" + rospl.getRoute() + RequestParameters.RIGHT_BRACKETS);
        }
    ...
    protected void deleteEntry(BasicPoolEntry entry) {
        HttpRoute route = entry.getPlannedRoute();
        if (this.log.isDebugEnabled()) {
            this.log.debug("Deleting connection [" + route + "][" + entry.getState() + RequestParameters.RIGHT_BRACKETS);
        }
        this.poolLock.lock();
        try {
            closeConnection(entry);
    ...
    protected void notifyWaitingThread(RouteSpecificPool rospl) {
        ...
        if (rospl != null) {
            try {
                if (rospl.hasThread()) {
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Notifying thread waiting on pool [" + rospl.getRoute() + RequestParameters.RIGHT_BRACKETS);
                    }
        ...
        try {
            if (!this.shutdown) {
                ...
                while (iter2.hasNext()) {
                    ...
                    iter2.remove();
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Closing connection [" + entry.getPlannedRoute() + "][" + entry.getState() + RequestParameters.RIGHT_BRACKETS);
                    }
                    closeConnection(entry);
                }
