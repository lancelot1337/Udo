...
public class CachingExec implements ClientExecChain {
    ...
    private void recordCacheMiss(HttpHost target, HttpRequestWrapper request) {
        ...
        if (this.log.isTraceEnabled()) {
            this.log.trace("Cache miss [host: " + target + "; uri: " + request.getRequestLine().getUri() + RequestParameters.RIGHT_BRACKETS);
        }
    }
    ...
    private void recordCacheHit(HttpHost target, HttpRequestWrapper request) {
        ...
        if (this.log.isTraceEnabled()) {
            this.log.trace("Cache hit [host: " + target + "; uri: " + request.getRequestLine().getUri() + RequestParameters.RIGHT_BRACKETS);
        }
    }
    ...
    private boolean explicitFreshnessRequest(HttpRequestWrapper request, HttpCacheEntry entry, Date now) {
        for (Header h : request.getHeaders(HeaderConstants.CACHE_CONTROL)) {
            for (HeaderElement elt : h.getElements()) {
                ...
                } else if (HeaderConstants.CACHE_CONTROL_MIN_FRESH.equals(elt.getName()) || ClientCookie.MAX_AGE_ATTR.equals(elt.getName())) {
                    return true;
                }
            }
        }
    ...
    private String generateViaHeader(HttpMessage msg) {
        ...
        if (HttpHost.DEFAULT_SCHEME_NAME.equalsIgnoreCase(pv.getProtocol())) {
            value = String.format("%d.%d localhost (Apache-HttpClient/%s (cache))", new Object[]{Integer.valueOf(major), Integer.valueOf(minor), release});
        } else {
            value = String.format("%s/%d.%d localhost (Apache-HttpClient/%s (cache))", new Object[]{pv.getProtocol(), Integer.valueOf(major), Integer.valueOf(minor), release});
        }
    ...
    boolean clientRequestsOurOptions(HttpRequest request) {
        ...
        if (HttpOptions.METHOD_NAME.equals(line.getMethod()) && "*".equals(line.getUri()) && AppEventsConstants.EVENT_PARAM_VALUE_NO.equals(request.getFirstHeader(HeaderConstants.MAX_FORWARDS).getValue())) {
            return true;
        }
        return false;
    }
    ...
    private boolean revalidationResponseIsTooOld(HttpResponse backendResponse, HttpCacheEntry cacheEntry) {
        Header entryDateHeader = cacheEntry.getFirstHeader(HTTP.DATE_HEADER);
    ...
    CloseableHttpResponse negotiateResponseFromVariants(HttpRoute route, HttpRequestWrapper request, HttpClientContext context, HttpExecutionAware execAware, Map<String, Variant> variants) throws IOException, HttpException {
        ...
        try {
            ...
            if (backendResponse.getStatusLine().getStatusCode() != HttpStatus.SC_NOT_MODIFIED) {
                return handleBackendResponse(request, context, requestDate, responseDate, backendResponse);
            }
    ...
    CloseableHttpResponse revalidateCacheEntry(HttpRoute route, HttpRequestWrapper request, HttpClientContext context, HttpExecutionAware execAware, HttpCacheEntry cacheEntry) throws IOException, HttpException {
        ...
        return handleBackendResponse(conditionalRequest, context, requestDate, responseDate, backendResponse);
    }
    ...
    private void storeRequestIfModifiedSinceFor304Response(HttpRequest request, HttpResponse backendResponse) {
        if (backendResponse.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_MODIFIED) {
            Header h = request.getFirstHeader(HeaderConstants.IF_MODIFIED_SINCE);
            if (h != null) {
                backendResponse.addHeader(HeaderConstants.LAST_MODIFIED, h.getValue());
            }
        }
    }
