...
class CacheValidityPolicy {
    ...
    public boolean isRevalidatable(HttpCacheEntry entry) {
        return (entry.getFirstHeader(HeaderConstants.ETAG) == null && entry.getFirstHeader(HeaderConstants.LAST_MODIFIED) == null) ? false : true;
    }
    ...
    public boolean mayReturnStaleIfError(HttpRequest request, HttpCacheEntry entry, Date now) {
        ...
        return mayReturnStaleIfError(request.getHeaders(HeaderConstants.CACHE_CONTROL), stalenessSecs) || mayReturnStaleIfError(entry.getHeaders(HeaderConstants.CACHE_CONTROL), stalenessSecs);
    }
    ...
    protected boolean hasContentLengthHeader(HttpCacheEntry entry) {
        return entry.getFirstHeader(HTTP.CONTENT_LEN) != null;
    }

    protected boolean contentLengthHeaderMatchesActualLength(HttpCacheEntry entry) {
        return !hasContentLengthHeader(entry) || (entry.getResource() != null && getContentLengthValue(entry) == entry.getResource().length());
    }
    ...
    protected long getMaxAge(HttpCacheEntry entry) {
        ...
        for (Header hdr : entry.getHeaders(HeaderConstants.CACHE_CONTROL)) {
            for (HeaderElement elt : hdr.getElements()) {
                if (ClientCookie.MAX_AGE_ATTR.equals(elt.getName()) || "s-maxage".equals(elt.getName())) {
                    try {
