...
class ResponseProtocolCompliance {
    ...
    private void identityIsNotUsedInContentEncoding(HttpResponse response) {
        ...
        if (hdrs != null && hdrs.length != 0) {
            ...
            for (Header h : hdrs) {
                ...
                if (!BuildConfig.FLAVOR.equals(newHeaderValue)) {
    ...
    private void ensurePartialContentIsNotSentToAClientThatDidNotRequestIt(HttpRequest request, HttpResponse response) throws IOException {
        if (request.getFirstHeader(HeaderConstants.RANGE) == null && response.getStatusLine().getStatusCode() == HttpStatus.SC_PARTIAL_CONTENT) {
            consumeBody(response);
    ...
    private void ensure200ForOPTIONSRequestWithNoBodyHasContentLengthZero(HttpRequest request, HttpResponse response) {
        if (request.getRequestLine().getMethod().equalsIgnoreCase(HttpOptions.METHOD_NAME) && response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && response.getFirstHeader(HTTP.CONTENT_LEN) == null) {
    ...
    private void ensure304DoesNotContainExtraEntityHeaders(HttpResponse response) {
        int i = 0;
        String[] disallowedEntityHeaders = new String[]{HeaderConstants.ALLOW, HTTP.CONTENT_ENCODING, HttpHeaders.CONTENT_LANGUAGE, HTTP.CONTENT_LEN, HttpHeaders.CONTENT_MD5, HeaderConstants.CONTENT_RANGE, HTTP.CONTENT_TYPE, HeaderConstants.LAST_MODIFIED};
    ...
    private boolean backendResponseMustNotHaveBody(HttpRequest request, HttpResponse backendResponse) {
        return HttpHead.METHOD_NAME.equals(request.getRequestLine().getMethod()) || backendResponse.getStatusLine().getStatusCode() == HttpStatus.SC_NO_CONTENT || backendResponse.getStatusLine().getStatusCode() == HttpStatus.SC_RESET_CONTENT || backendResponse.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_MODIFIED;
    }
