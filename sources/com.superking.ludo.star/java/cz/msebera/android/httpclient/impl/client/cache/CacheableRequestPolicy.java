...
class CacheableRequestPolicy {
    ...
    public boolean isServableFromCache(HttpRequest request) {
        ...
        if (HttpVersion.HTTP_1_1.compareToVersion(request.getRequestLine().getProtocolVersion()) != 0) {
            ...
            return false;
        } else if (!method.equals(HttpGet.METHOD_NAME) && !method.equals(HttpHead.METHOD_NAME)) {
            this.log.trace("non-GET or non-HEAD request was not serveable from cache");
            return false;
        } else if (request.getHeaders(HeaderConstants.PRAGMA).length > 0) {
            this.log.trace("request with Pragma header was not serveable from cache");
            return false;
        } else {
            for (Header cacheControl : request.getHeaders(HeaderConstants.CACHE_CONTROL)) {
