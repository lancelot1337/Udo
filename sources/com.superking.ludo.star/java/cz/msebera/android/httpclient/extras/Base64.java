...
public class Base64 {
            ...
            while (p < len) {
                if (state == 0) {
                    while (p + Base64.CRLF <= len) {
                        value = (((alphabet[input[p] & 255] << 18) | (alphabet[input[p + Base64.NO_PADDING] & 255] << 12)) | (alphabet[input[p + Base64.NO_WRAP] & 255] << 6)) | alphabet[input[p + 3] & 255];
                        if (value >= 0) {
    ...
    static class Encoder extends Coder {
        ...
        public Encoder(int flags, byte[] output) {
            ...
            this.do_newline = (flags & Base64.NO_WRAP) == 0 ? true : $assertionsDisabled;
...
*/
    ...
    public static String encodeToString(byte[] input, int flags) {
        try {
            return new String(encode(input, flags), HTTP.US_ASCII);
    ...
    public static String encodeToString(byte[] input, int offset, int len, int flags) {
        try {
            return new String(encode(input, offset, len, flags), HTTP.US_ASCII);
        } catch (UnsupportedEncodingException e) {
            throw new AssertionError(e);
        }
    }
    ...
    public static byte[] encode(byte[] input, int offset, int len, int flags) {
        ...
        if (encoder.do_newline && len > 0) {
            output_len += (encoder.do_cr ? NO_WRAP : NO_PADDING) * (((len - 1) / 57) + NO_PADDING);
        }
