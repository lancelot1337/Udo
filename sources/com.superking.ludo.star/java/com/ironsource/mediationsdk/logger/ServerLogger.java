...
public class ServerLogger extends IronSourceLogger {
    ...
    private final int SERVER_LOGS_SIZE_LIMIT = GameControllerDelegate.THUMBSTICK_LEFT_X;
    ...
    private synchronized void addLogEntry(ServerLogEntry entry) {
        ...
        if (shouldSendLogs()) {
            send();
        } else if (this.mLogs.size() > GameControllerDelegate.THUMBSTICK_LEFT_X) {
            try {
                ArrayList<ServerLogEntry> newerLog = new ArrayList();
                for (int i = HttpStatus.SC_INTERNAL_SERVER_ERROR; i < this.mLogs.size(); i++) {
