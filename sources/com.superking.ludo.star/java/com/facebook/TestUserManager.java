...
public class TestUserManager {
    ...
    private synchronized void retrieveTestAccountsForAppIfNeeded() {
        if (this.appTestAccounts == null) {
            ...
            testUserNamesParam.putString(GraphRequest.FIELDS_PARAM, MediationMetaData.KEY_NAME);
            ...
            populateTestAccounts(((GraphResponse) responses.get(0)).getJSONObject().optJSONArray(EventEntry.COLUMN_NAME_DATA), ((GraphResponse) responses.get(1)).getJSONObject());
        }
    }
    ...
    private synchronized void populateTestAccounts(JSONArray testAccounts, JSONObject userAccountsMap) {
        for (int i = 0; i < testAccounts.length(); i++) {
            ...
            try {
                testAccount.put(MediationMetaData.KEY_NAME, userAccountsMap.optJSONObject(testAccount.optString(ShareConstants.WEB_DIALOG_PARAM_ID)).optString(MediationMetaData.KEY_NAME));
    ...
    private synchronized JSONObject findTestAccountMatchingIdentifier(String identifier) {
        ...
        for (JSONObject testAccount2 : this.appTestAccounts.values()) {
            if (testAccount2.optString(MediationMetaData.KEY_NAME).contains(identifier)) {
                break;
            }
        }
        testAccount2 = null;
    ...
    private JSONObject createTestAccount(List<String> permissions, Mode mode, String uniqueUserTag) {
        ...
        parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, getAppAccessToken());
        if (mode == Mode.SHARED) {
            parameters.putString(MediationMetaData.KEY_NAME, String.format("Shared %s Testuser", new Object[]{getSharedTestAccountIdentifier(permissions, uniqueUserTag)}));
        }
        ...
        if (!$assertionsDisabled && testAccount == null) {
            ...
            try {
                testAccount.put(MediationMetaData.KEY_NAME, parameters.getString(MediationMetaData.KEY_NAME));
            } catch (JSONException e) {
                Log.e(LOG_TAG, "Could not set name", e);
            }
            storeTestAccount(testAccount);
