...
public class DeviceShareDialog extends FacebookDialogBase<ShareContent, Result> {
    ...
    protected void showImpl(ShareContent content, Object mode) {
        if (content == null) {
            ...
            intent.putExtra(Param.CONTENT, content);
        ...
        callbackManager.registerCallback(getRequestCode(), new Callback() {
            public boolean onActivityResult(int resultCode, Intent data) {
                if (data.hasExtra(NativeProtocol.BRIDGE_ARG_ERROR_BUNDLE)) {
                    callback.onError(((FacebookRequestError) data.getParcelableExtra(NativeProtocol.BRIDGE_ARG_ERROR_BUNDLE)).getException());
                } else {
