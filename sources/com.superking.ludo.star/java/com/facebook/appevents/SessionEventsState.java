...
class SessionEventsState {
    private final int MAX_ACCUMULATED_LOG_EVENTS = GameControllerDelegate.THUMBSTICK_LEFT_X;
    ...
    public synchronized void addEvent(AppEvent event) {
        if (this.accumulatedEvents.size() + this.inFlightEvents.size() >= GameControllerDelegate.THUMBSTICK_LEFT_X) {
